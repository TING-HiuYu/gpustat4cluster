...
require_binaries
IFS='|' read -r dir _tcp_port udp_port _multicast inventory server_config client_config uds count < <(make_single_fixture "server-first-dynamic-udp" 24 udp)
...
echo "server-first-dynamic-udp ok nodes=1 gres=$count"
